...
export default function GetReplies(isOpen: boolean, id: number | string) {
  // const { mutate } = useSWRConfig();
  const token = Cookies.get("user_token"); // Get the token from cookies

  const { data, error, isLoading } = useSWR(
    token && isOpen === true ? [`${process.env.NEXT_PUBLIC_API_URL}/replies/post/${id}`, token] : null, // Conditional fetch
